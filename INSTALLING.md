...
export BABELFISH_HOME=/opt/babelfish/2.1
...
```sh
sudo mkdir -p /var/lib/babelfish/2.1
...
```sh
${BABELFISH_HOME}/bin/initdb -D /var/lib/babelfish/2.1/data/ -E "UTF8" --auth=trust --auth-host=trust --auth-local=trust
```
...
```sh
${BABELFISH_HOME}/bin/initdb -D /var/lib/babelfish/2.1/data/ -E "UTF8"
ipaddress=$(ifconfig eth0 | grep 'inet ' | cut -d: -f2 | awk '{ print $2}')
echo "host    all             all             $ip_address/32            trust" >> /var/lib/babelfish/2.1/data/pg_hba.conf
```
...
```sh
cat << EOF >> /var/lib/babelfish/2.1/data/postgresql.conf
...
```sh
${BABELFISH_HOME}/bin/pg_ctl -D /var/lib/babelfish/2.1/data/ -l logfile start
```
